...
test_forecast_availability() {
  local run_url="$1" hh atmos f000 f006 f000_status f006_status
  [[ -z "$run_url" ]] && return 2
  ...
  f000_status=$(http_status "$f000")
  f006_status=$(http_status "$f006")
  echo "Check: $f000 -> $f000_status"
  echo "Check: $f006 -> $f006_status"
  [[ "$f000_status" == "200" && "$f006_status" == "200" ]]
}
...
process_with_python() {
  ...
  if [[ "$STORAGE_MODE" == "s3" ]]; then
    ...
    python_cmd="$python_cmd --download ./downloads"
  else
